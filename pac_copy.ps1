...
# ----------------- DYNAMIC PAC FILE DISCOVERY (UPDATED) --------------------
# List of PAC file OBJECTS to monitor and synchronize. This is now dynamically generated
# by checking all .pac files in the repository folder AND ALL SUBFOLDERS.
$PACFileObjects = @(Get-ChildItem -Path $RepositoryFolder -Filter "*.pac" -File -Recurse)

if ($PACFileObjects.Count -eq 0) {
    Write-Log "ERROR: No PAC files found in the repository folder: $RepositoryFolder or its subfolders. Exiting script." "ERROR"
    exit 1
}
Write-Log "Discovered PAC files to process: $($PACFileObjects.Name -join ', ')"
# -----------------------------------------------------------------
...
foreach ($FileObject in $PACFileObjects) {
    # Extract file details from the object
    $PACFile = $FileObject.Name
    $RepoFile = $FileObject.FullName # <--- Use the FullName (path) from the recursive search
...
        Write-Log "PAC file '$PACFile' found in Repository at '$RepoFile'. Last modified: $RepoLastModified"
...
            Write-Log "Deployed new '$PACFile' from $RepoFile to primary live folder: $LiveFile"
